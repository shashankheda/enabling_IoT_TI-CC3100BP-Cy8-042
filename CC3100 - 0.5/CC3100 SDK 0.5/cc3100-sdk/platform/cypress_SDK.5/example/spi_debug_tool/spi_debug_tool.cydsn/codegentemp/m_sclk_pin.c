...
#define SetP4PinDriveMode(shift, mode)  \
    do { \
        m_sclk_pin_PC =   (m_sclk_pin_PC & \
                                (uint32)(~(uint32)(m_sclk_pin_DRIVE_MODE_IND_MASK << (m_sclk_pin_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << (m_sclk_pin_DRIVE_MODE_BITS * (shift))); \
    } while (0)
...
{
    uint8 drVal = (uint8)(m_sclk_pin_DR & (uint8)(~m_sclk_pin_MASK));
    drVal = (drVal | ((uint8)(value << m_sclk_pin_SHIFT) & m_sclk_pin_MASK));
    m_sclk_pin_DR = (uint32)drVal;
}
...
{
	SetP4PinDriveMode(m_sclk_pin__0__SHIFT, mode);
}
...
{
    return (uint8)((m_sclk_pin_PS & m_sclk_pin_MASK) >> m_sclk_pin_SHIFT);
}
...
{
    return (uint8)((m_sclk_pin_DR & m_sclk_pin_MASK) >> m_sclk_pin_SHIFT);
}
...
/* If Interrupts Are Enabled for this Pins component */ 
    ...
    *******************************************************************************/
    ...
    {
		uint8 maskedStatus = (uint8)(m_sclk_pin_INTSTAT & m_sclk_pin_MASK);
		m_sclk_pin_INTSTAT = maskedStatus;
        return maskedStatus >> m_sclk_pin_SHIFT;
    }
